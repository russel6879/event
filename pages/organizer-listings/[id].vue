<template>
    <section class="breadcrumb-area bread-bg">
      ...
      <div class="container">
        <div class="breadcrumb-content text-center">
          <h2 class="sec__title text-white mb-3">Organizer Listings</h2>
          <ul class="bread-list">
            <li><NuxtLink to="/">home</NuxtLink></li>
            <li>listings</li>
            <li>{{ userName }}</li> <!-- Change to display user name -->
          </ul>
        </div>
      </div>
      <div class="bread-svg">
  ...
  const getUserEvents = async (page = 1) => {
    try {
      const userId = route.params.id; // Get the user ID from the URL
  ...
  watch(() => route.query.page, (newPage) => {
    getUserEvents(Number(newPage) || 1);
  });
